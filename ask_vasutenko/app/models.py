...
class Question(models.Model):
    ...
    def update_rating(self):
        self.rating = self.rates.filter(is_dislike=False).count() - self.rates.filter(is_dislike=True).count()
...
class QuestionRate(models.Model):
    ...
    question = models.ForeignKey('Question', on_delete=models.CASCADE, verbose_name='Вопрос', related_name='rates', db_index=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='Пользователь', related_name='question_rates', db_index=True)
...
class AnswerRate(models.Model):
    is_dislike = models.BooleanField(verbose_name='Дизлайк', default=False, db_index=True)
    answer = models.ForeignKey('Answer', on_delete=models.CASCADE, verbose_name='Ответ', related_name='rates', db_index=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='Пользователь', related_name='answer_rates', db_index=True)
    created_at = models.DateTimeField('Поставлен', auto_now_add=True, db_index=True)
